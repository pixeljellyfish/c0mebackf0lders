...
if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"aphelion://"]]) {
	...
	} else if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"twitter.com//"]]) {
		return [@"twitter.com//user?screen_name=" stringByAppendingString:user];
	} else {
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier specifier:(PSSpecifier *)specifier 
{
	...
	if (self) {
		...
		UIImage *avatarImageOne = [UIImage imageNamed:[NSString stringWithFormat:@"/Library/PreferenceBundles/cbfprefs.bundle/%@.png", specifier.properties[@"firstAccount"]]];
		UIImage *avatarImageTwo = [UIImage imageNamed:[NSString stringWithFormat:@"/Library/PreferenceBundles/cbfprefs.bundle/%@.png", specifier.properties[@"secondAccount"]]];
		...
		if (@available(iOS 13, *)) 
		{
			displayNameOne.textColor = [UIColor cyanColor];
			displayNameTwo.textColor = [UIColor cyanColor];
			accountNameOne.textColor = [UIColor whiteColor];
